...
TEST_F(GameTest, MoveLeftBlockedByWall) {
    ...
    restart_and_play();
    for (int i = 0; i < 3; ++i) game_.process(GameAction::MoveLeft);
...
TEST_F(GameTest, RotateFullCircle) {
    restart_and_play();
    auto init_rot = game_.current_piece()->rotation();
    for (int i = 0; i < 4; ++i) game_.process(GameAction::RotateCW);
...
TEST_F(GameTest, GhostBelowOrAtCurrent) {
    ...
    int ghost_max_row = 0;
    for (auto& p : ghost) ghost_max_row = std::max(ghost_max_row, static_cast<int>(p.row));
    int cells_max_row = 0;
    for (auto& p : cells) cells_max_row = std::max(cells_max_row, static_cast<int>(p.row));
...
TEST_F(GameTest, RestartResetsEverything) {
    restart_and_play();
    game_.process(GameAction::HardDrop);
    if (game_.state() == GameState::Playing) game_.process(GameAction::HardDrop);

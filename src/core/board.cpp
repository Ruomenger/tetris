...
bool Board::is_in_bounds(Position pos) const noexcept {
    return pos.col >= 0 && pos.col < kBoardWidth
        && pos.row >= 0 && pos.row < kBoardTotalHeight;
}

std::optional<TetrominoType> Board::cell_at(Position pos) const noexcept {
    if (!is_in_bounds(pos)) return std::nullopt;
...
bool Board::can_place(std::span<const Position> cells) const noexcept {
    return std::ranges::all_of(cells, [this](Position p) {
        return is_in_bounds(p) && !is_occupied(p);
    });
}
...
std::expected<uint8_t, BoardError> Board::clear_lines() {
    ...
    for (int8_t row = kBoardTotalHeight - 1; row >= 0; --row) {
        bool full = std::ranges::all_of(
            grid_[static_cast<size_t>(row)],
